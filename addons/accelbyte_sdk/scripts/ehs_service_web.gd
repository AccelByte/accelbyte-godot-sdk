...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [EhsServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [EhsServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
## Get gRPC services list using reflection
## GET /ehs/v1/admin/namespaces/{namespace}/reflection
func get_grpcreflection_info(namespace_param: String,app_name: String = "",host: String = "",security_type: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/ehs/v1/admin/namespaces/{namespace}/reflection"
...
## Get a token for pushing container images in a publisher game app.
## GET /ehs/v1/namespaces/{namespace}/apps/{app}/token
func get_upload_token_v1(app: String,namespace_param: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/ehs/v1/namespaces/{namespace}/apps/{app}/token"
...
## GET /ehs/version
func version_checker_handler() -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/ehs/version"
