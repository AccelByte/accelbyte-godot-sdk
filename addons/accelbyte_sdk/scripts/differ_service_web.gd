...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [DifferServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [DifferServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
func perform_late_diff(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
...
func ping(namespace_param: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
func perform_late_diff_v2(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
