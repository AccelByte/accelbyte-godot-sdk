...
##
## Usage:
##   var service = GametelemetryServiceWeb.new()
##   service.initialize(sdk)  # Pass your AccelByteSDK instance
##   var result = await service.method_name(params)
## =============================================================================

class_name GametelemetryServiceWeb
extends RefCounted
...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [GametelemetryServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [GametelemetryServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
func get_events_game_telemetry_v1_admin_namespaces__namespace__events_get(namespace_param: String,start_time: String = "",end_time: String = "",offset: int = 0,limit: int = 0,user_id: String = "",flight_id: String = "",event_id: String = "",event_name: String = "",event_payload: String = "",device_type: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
func protected_save_events_game_telemetry_v1_protected_events_post(cookie: String = "",
		body: Array = []
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
func protected_get_playtime_game_telemetry_v1_protected_steam_ids__steam_id__playtime_get(steam_id: String,cookie: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	url_path = url_path.replace("{" + "steamId" + "}", _url_encode(steam_id))
...
func protected_update_playtime_game_telemetry_v1_protected_steam_ids__steam_id__playtime__playtime__put(steam_id: String,playtime: String,cookie: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	url_path = url_path.replace("{" + "steamId" + "}", _url_encode(steam_id))
	url_path = url_path.replace("{" + "playtime" + "}", _url_encode(playtime))
