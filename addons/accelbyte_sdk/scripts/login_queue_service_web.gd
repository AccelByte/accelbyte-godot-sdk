...
##
## Usage:
##   var service = LoginQueueServiceWeb.new()
##   service.initialize(sdk)  # Pass your AccelByteSDK instance
##   var result = await service.method_name(params)
## =============================================================================

class_name LoginQueueServiceWeb
extends RefCounted
...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [LoginQueueServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [LoginQueueServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
func admin_get_configuration(namespace_param: String
) -> Dictionary:
...
func admin_update_configuration(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
func admin_empty_login_queue(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	# Build URL path
	var url_path: String = "/login-queue/v1/admin/namespaces/{namespace}/empty"
	url_path = url_path.replace("{" + "namespace" + "}", _url_encode(namespace_param))
	# Replace namespace
	if url_path.contains("{namespace}") and not _namespace.is_empty():
		url_path = url_path.replace("{namespace}", _url_encode(_namespace))
...
func admin_get_status(namespace_param: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
## Create ticket
## POST /login-queue/v1/internal/namespaces/{namespace}/ticket
func create_ticket(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	# Build URL path
	var url_path: String = "/login-queue/v1/internal/namespaces/{namespace}/ticket"
	url_path = url_path.replace("{" + "namespace" + "}", _url_encode(namespace_param))
	# Replace namespace
	if url_path.contains("{namespace}") and not _namespace.is_empty():
		url_path = url_path.replace("{namespace}", _url_encode(_namespace))
...
## Validate token
## POST /login-queue/v1/internal/namespaces/{namespace}/token/validate
func validate_token(namespace_param: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	# Build URL path
	var url_path: String = "/login-queue/v1/internal/namespaces/{namespace}/token/validate"
	url_path = url_path.replace("{" + "namespace" + "}", _url_encode(namespace_param))
	# Replace namespace
	if url_path.contains("{namespace}") and not _namespace.is_empty():
		url_path = url_path.replace("{namespace}", _url_encode(_namespace))
...
func cancel_ticket(namespace_param: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
...
func refresh_ticket(namespace_param: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
