...
## Service: Justice QoS Manager Service
## Version: 
...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [QosmServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [QosmServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
## Update QoS Service configuration
## PATCH /qosm/admin/namespaces/{namespace}/servers/{region}
func update_server_config(namespace_param: String,region: String,
		body: Dictionary = {}
) -> Dictionary:
	...
	# Build URL path
	var url_path: String = "/qosm/admin/namespaces/{namespace}/servers/{region}"
...
## Delete a QoS service record
## DELETE /qosm/admin/servers/{region}
func delete_server(region: String
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/qosm/admin/servers/{region}"
	url_path = url_path.replace("{" + "region" + "}", _url_encode(region))
...
## Modify a QoS service's region alias
## POST /qosm/admin/servers/{region}/alias
func set_server_alias(region: String,
		body: Dictionary = {}
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()
	...
	# Build URL path
	var url_path: String = "/qosm/admin/servers/{region}/alias"
	url_path = url_path.replace("{" + "region" + "}", _url_encode(region))
...
## Get list of QoS services per region
## GET /qosm/public/namespaces/{namespace}/qos
func list_server_per_namespace(namespace_param: String,status: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/qosm/public/namespaces/{namespace}/qos"
	url_path = url_path.replace("{" + "namespace" + "}", _url_encode(namespace_param))
	# Replace namespace
	if url_path.contains("{namespace}") and not _namespace.is_empty():
		url_path = url_path.replace("{namespace}", _url_encode(_namespace))
...
## Get list of QoS services
## GET /qosm/public/qos
func list_server() -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/qosm/public/qos"
...
func heartbeat(
	...
	# Build URL path
	var url_path: String = "/qosm/servers/heartbeat"
