...
func _http_request(url: String, method: int, headers: PackedStringArray, body: String = "") -> Dictionary:
	print("  [AmsQosmServiceWeb] %s %s" % [_method_to_string(method), url])
	...
	print("  [AmsQosmServiceWeb] Response: %d - %s" % [response_code, "success" if result["success"] else "failed"])
	return result
...
## GET /ams-qosm/public/namespaces/{namespace}/qos
func func3(namespace_param: String,status: String = ""
) -> Dictionary:
	# Update auth token from SDK
	update_auth_token()

	# Build URL path
	var url_path: String = "/ams-qosm/public/namespaces/{namespace}/qos"
